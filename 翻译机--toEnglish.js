...
// @version      0.42
// @description  该脚本用于翻译各类常用社交网站为英文，不会经过中间服务器。
// @author       HolynnChen
// @match        *://*.twitter.com/*
// @match        *://*.youtube.com/*
// @match        *://*.facebook.com/*
...
async function pass_lang(raw){//确认是否为中文，是则中断promise
    ...
    try{
        ...
        if(result=='en')return new Promise(()=>{});
        return result
    }catch(err){
        console.log(err);
        return
...
async function translate_gg(raw){
    const options = {
        ...
        data: "f.req="+encodeURIComponent(JSON.stringify([[["MkEWBc",JSON.stringify([[raw,"auto","en",true],[null]]),null,"generic"]]])),
        headers: {
            "content-type": "application/x-www-form-urlencoded",
